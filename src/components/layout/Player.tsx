  ...
  return (
    <>
      <motion.div
        initial={{ y: 80 }}
        animate={{ y: 0 }}
        className="fixed bottom-16 left-0 right-0 z-50 border-t border-white/8 bg-black/80 px-3 py-3 backdrop-blur-xl md:bottom-0 md:left-72 md:px-6"
      >
        <div className="flex items-center gap-3 md:gap-6">
          ...
          >
            ...
            <div className="min-w-0">
              ...
              <p className="hidden text-[11px] uppercase tracking-[0.28em] text-zinc-500 md:block">{trackBadge}</p>
            </div>
          </button>
          ...
          <div className="flex flex-1 flex-col items-center gap-2">
            <div className="flex items-center gap-3 md:gap-5">
              <Button
                ...
                className={cn('hidden rounded-full md:inline-flex', isShuffle && 'bg-white/10 text-gamero-lime')}
              >
              ...
              <Button
                ...
                size="icon"
                onClick={cycleRepeat}
                className={cn('hidden rounded-full md:inline-flex', repeatMode !== 'off' && 'bg-white/10 text-gamero-lime')}
                title={repeatLabel}
              >
            ...
            <div className="hidden w-full max-w-xl items-center gap-3 md:flex">
              <span className="w-10 text-right text-[11px] text-zinc-500">{formatTime((progress / 100) * currentTrack.duration)}</span>
              <Slider value={[progress]} max={100} step={0.1} onValueChange={(values) => seek(values[0])} />
              <span className="w-10 text-[11px] text-zinc-500">{formatTime(currentTrack.duration)}</span>
            </div>
          </div>
          ...
          <div className="hidden items-center gap-3 lg:flex">
            <Button
              ...
              className={cn('rounded-full', isSavedTrack(currentTrack.id) && 'text-gamero-lime')}
            >
            ...
            <Button variant="ghost" size="icon" onClick={() => setIsQueueOpen((current) => !current)} className="relative rounded-full">
              ...
              {activeQueue.length > 0 && (
                <span className="absolute -right-1 -top-1 flex h-5 min-w-5 items-center justify-center rounded-full bg-white/10 px-1 text-[10px] font-medium text-zinc-200">
      ...
      {isExpanded && (
        <div className="fixed inset-0 z-[70] bg-[radial-gradient(circle_at_top,_rgba(147,248,114,0.18),_transparent_40%),linear-gradient(180deg,_#0d1324_0%,_#050816_100%)] px-5 py-8">
          <div className="mx-auto flex h-full max-w-6xl flex-col">
            <div className="flex items-center justify-between">
              <Button variant="ghost" size="icon" onClick={() => setIsExpanded(false)} className="rounded-full">
                <ChevronDown className="h-6 w-6" />
              </Button>
              <p className="text-[11px] uppercase tracking-[0.32em] text-zinc-500">{trackBadge}</p>
            </div>
            ...
            <div className="flex flex-1 flex-col items-center justify-center gap-10 md:flex-row md:items-stretch">
              ...
              <div className="flex w-full max-w-2xl flex-col justify-center gap-8">
                <div>
                  <p className="text-[11px] uppercase tracking-[0.32em] text-zinc-500">{currentTrack.genre || 'Discovery'}</p>
                  <h2 className="mt-3 font-display text-4xl font-semibold md:text-6xl">{currentTrack.title}</h2>
                  <p className="mt-2 text-xl text-zinc-300">{currentTrack.artist}</p>
                  {currentTrack.description && <p className="mt-5 max-w-xl text-base leading-7 text-zinc-400">{currentTrack.description}</p>}
                </div>
                ...
                <div className="flex items-center gap-4 md:gap-6">
                  <Button
                    ...
                    size="icon"
                    onClick={toggleShuffle}
                    className={cn('rounded-full', isShuffle && 'bg-white/10 text-gamero-lime')}
                  >
                  ...
                  <Button
                    ...
                    size="icon"
                    onClick={cycleRepeat}
                    className={cn('rounded-full', repeatMode !== 'off' && 'bg-white/10 text-gamero-lime')}
                    title={repeatLabel}
                  >
                ...
                <div className="flex flex-wrap items-center gap-3 text-sm text-zinc-400">
                  <span className={cn('rounded-full border px-4 py-2', isShuffle ? 'border-gamero-lime/35 bg-gamero-lime/10 text-gamero-lime' : 'border-white/10')}>
                    {isShuffle ? 'Shuffle on' : 'Shuffle off'}
                  </span>
                  <span className={cn('rounded-full border px-4 py-2', repeatMode !== 'off' ? 'border-gamero-lime/35 bg-gamero-lime/10 text-gamero-lime' : 'border-white/10')}>
                    {repeatLabel}
                  </span>
                ...
                <div className="rounded-[28px] border border-white/8 bg-white/[0.04] p-6">
                  <p className="text-[11px] uppercase tracking-[0.32em] text-zinc-500">Lyrics / Notes</p>
                  <div className="mt-5 space-y-3 text-lg leading-8 text-zinc-300">
                    {currentTrack.lyrics ? (
      ...
      {isQueueOpen && (
        <div className="fixed bottom-36 right-4 z-[72] w-[min(26rem,calc(100vw-2rem))] rounded-[30px] border border-white/10 bg-[#09111f]/96 p-4 shadow-[0_24px_80px_rgba(0,0,0,0.35)] backdrop-blur-xl md:bottom-28 md:right-6">
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="text-xs uppercase tracking-[0.28em] text-zinc-500">{queueTitle}</p>
              <h3 className="mt-2 font-display text-2xl font-semibold text-white">{queueSubtitle}</h3>
            </div>
          ...
          <div className="mt-4 max-h-[50vh] space-y-2 overflow-y-auto pr-1">
            {activeQueue.slice(0, 12).map((track) => {
              ...
              return (
                <div key={track.id} className="flex items-center gap-3 rounded-[22px] border border-white/6 bg-white/[0.03] p-3">
                  ...
                  {queue.length > 0 && (
                    <Button variant="ghost" size="icon" onClick={() => removeFromQueue(track.id)} className="rounded-full text-zinc-400 hover:text-white">
                      <X className="h-4 w-4" />
                    </Button>
                  )}
                </div>
